...
try:
    ...
    # Aggregate old data if any
    conn.execute("""
      INSERT INTO user_history_new (user_id, bookmark_id, click_count, first_clicked_at, last_clicked_at)
      SELECT user_id, bookmark_id, COUNT(*), MIN(created_at), MAX(created_at)
      FROM user_history
      GROUP BY user_id, bookmark_id
    """)
    ...
    c = conn.cursor()
    rows = c.execute("SELECT COUNT(*) FROM user_history").fetchone()[0]
    print(f"Rows after migration: {rows}")
